---
title: "Research"
layout: single
permalink: /research/
---

## Interests
Phonology, poetry, Basque, historical linguistics.

### Narrower interests
Rhyme as harmony, alliteration etc as rhyme, subregular models of phonological maps, origins of Basque, cognitive requirements of Basque improvised poetry duels (bertsolaritza).

### Poster from LSA 2026

<div style="width:100%; text-align:center;">
  <a href="{{ '/assets/lsa.pdf' | relative_url }}" target="_blank">
    <img src="{{ '/assets/img/lsa.png' | relative_url }}"
         style="width:100%; max-width:1600px; height:auto; border:1px solid #ccc;">
  </a>
</div>

<p style="text-align:center;"><em>Click the poster to open the full PDF.</em></p>

### Poster from MidPhon 2025

<div style="width:100%; text-align:center;">
  <a href="{{ '/assets/midphon.pdf' | relative_url }}" target="_blank">
    <img src="{{ '/assets/img/midphon.png' | relative_url }}"
         style="width:100%; max-width:1600px; height:auto; border:1px solid #ccc;">
  </a>
</div>

<p style="text-align:center;"><em>Click the poster to open the full PDF.</em></p>
